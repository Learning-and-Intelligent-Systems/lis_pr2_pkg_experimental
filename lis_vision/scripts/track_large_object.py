...
# Author: Ariel Anders
"""
Basic filter for largest object detected and table
publishes results as collision objects for use in moveit
"""
...
from geometry_msgs.msg import\
        PoseStamped, Point, Pose, Quaternion, Vector3
...
class Tracker:
    ...
    def __init__(self, debug=False, tf_listener=None):
        ...
        self.sub_table = rospy.Subscriber(\
                "/best_table", Table, \
                self.cb_obj,callback_args="table", queue_size=1)
    
        # keep track of object
        self.best_obj = None
        self.sub_bb = rospy.Subscriber(\
                "/bounding_boxes", MarkerArray, self.cb_obj,\
                callback_args="bb",queue_size=1)

        self.pub = rospy.Publisher(\
                "/table_and_object", MarkerArray, queue_size=2)
        self.pub_co = rospy.Publisher(\
                "/ari_collision_object", CollisionObject, queue_size=1)
    ...
    def get_mean(self, obj_type, pts, dim):
        ...
        if np.mean(self.arr[obj_type]) == float('inf'):
            ...
            filtered_pos, filtered_dim = reject_outliers(\
                    self.arr[obj_type], self.arr_dims[obj_type])
